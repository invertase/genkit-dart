...
/// Defines a flow that returns the raw JSON transcription object.
///
/// Uses `responseFormat: 'verbose_json'` so the raw response map includes
/// language, duration, and segment metadata alongside the transcript text.
Flow<String, Map<String, dynamic>, void, void> defineWhisperJsonFlow(
  Genkit ai, {
  String model = 'gpt-4o-transcribe',
}) {
  return ai.defineFlow(
    name: 'whisperTranscribeJson',
    inputSchema: .string(
      defaultValue: 'data:audio/wav;base64,<base64-audio-bytes>',
    ),
    ...
    fn: (audioDataUrl, _) => _generateTranscriptionJsonFromDataUrl(
      ...
      options: OpenAISttOptions(responseFormat: 'json'),
    ),
  );
}

/// Defines a flow that returns a verbose transcription with word and segment
/// timestamps.
///
/// Uses `responseFormat: 'verbose_json'` with
/// `timestampGranularities: ['word', 'segment']`, producing a response that
/// includes `language`, `duration`, `text`, per-segment metadata, and
/// per-word start/end times. Only supported by `whisper-1` — gpt-4o-transcribe
/// variants only accept `json` or `text`.
...
/// Defines a flow that transcribes audio using a GPT transcription model and
/// returns the raw JSON response object.
///
/// Uses `gpt-4o-transcribe` with `responseFormat: 'json'`. Note that
/// `verbose_json` is not supported by gpt-4o-transcribe variants — use
/// [defineWhisperVerboseTimestampFlow] with `whisper-1` for segment/word
/// timestamps.
Flow<String, Map<String, dynamic>, void, void> defineGptTranscribeJsonFlow(
  Genkit ai, {
  String model = 'gpt-4o-transcribe',
}) {
  return ai.defineFlow(
    name: 'gptTranscribeJson',
    inputSchema: .string(
      defaultValue: 'data:audio/wav;base64,<base64-audio-bytes>',
    ),
    outputSchema: .map(.string(), .dynamicSchema()),
...
Flow<String, String, void, void> defineWhisperTranslationFlow(
  ...
  return ai.defineFlow(
    ...
    fn: (audioDataUrl, _) => _generateTranscriptFromDataUrl(
      ...
      options: OpenAISttOptions(translate: true),
    ),
  );
}
...
void main() {
  ...
  defineWhisperJsonFlow(ai);
  defineWhisperVerboseTimestampFlow(ai);
  defineGptTranscribeJsonFlow(ai);
